...
export default function MasshHome() {
  return <div className="massh-site">
    ...
    <main id="top">
      ...
      <section className="section videos"><div className="shell"><Heading>Videos</Heading><CardSlider label="videos" className="video-grid">{surgeries.map(([name,icon],i)=><div className="video-card" key={name}><div className="video-art">{icon}<button aria-label={`Play ${name}`}>▶</button></div><span>{i%2 ? "Advanced Care at MASSH" : name}</span></div>)}</CardSlider></div></section>
      ...
      <section className="section blogs"><div className="shell"><Heading>Health Blogs</Heading><CardSlider label="health blogs" className="card-grid">{blogs.map(([title,img])=><article className="blog-card" key={title}><img src={img} alt=""/><h3>{title}</h3><p>By MASSH · 2026</p><button>Read More</button></article>)}</CardSlider><div className="explore"><button>● Explore More</button></div></div></section>
    </main>
    <SiteFooter />
  </div>;
}
